...
@torch.no_grad()
def pinball_loss_ensemble_joint(Y_true, Y_samp, quantiles):
    """
    Y_true: [B,T,N]
    Y_samp: [S,B,T,N]
    """
    ...
    if not torch.is_tensor(Y_samp):
        Y_samp = torch.as_tensor(Y_samp)
...
class Runner_gan_joint:
    def __init__(
        ...
        self.sup_samples = int(sup_samples)
        ...
        self.val_n_samples = int(val_n_samples)
        self.val_z_temp = float(val_z_temp)
        self.val_share_znode = bool(val_share_znode)
        self.val_quantiles = list(val_quantiles) if val_quantiles is not None else [0.05 * i for i in range(1, 20)]

        self.device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")

        input_dim = int(train_set.X.shape[-1])
        seq_len = int(train_set.X.shape[1])

        self.G = CGAN_Generator_Multi(
            input_dim=input_dim,
            n_nodes=self.n_nodes,
            seq_len=seq_len,
            z_global_dim=z_global_dim,
            z_node_dim=z_node_dim,
        ...
        self.D = CGAN_Discriminator_Multi(
            input_dim=input_dim,
            n_nodes=self.n_nodes,
            seq_len=seq_len,
            hidden=trunk_hidden,
        ...
        self.opt_g = torch.optim.Adam(self.G.parameters(), lr=float(lr_g), betas=(0.0, 0.9))
        self.opt_d = torch.optim.Adam(self.D.parameters(), lr=float(lr_d), betas=(0.0, 0.9))
    ...
    def _sample_latents(self, B, temp=1.0):
        zg = torch.randn(B, self.G.zg, device=self.device) * float(temp)
        zn = torch.randn(B, self.n_nodes, self.G.zn, device=self.device) * float(temp)
        return zg, zn

    def _sup_weight(self, ep):
        if self.lambda_sup <= 0.0:
            return 0.0
        if self.sup_warmup_epochs <= 0:
            return float(self.lambda_sup)
        w = min(max(ep / float(self.sup_warmup_epochs), 0.0), 1.0)
    ...
    @torch.no_grad()
    def evaluate_val_pinball(self, batch_size=256):
        self.G.eval()
        ...
        for batch in loader:
            ...
            samp = self.G.sample(
                X, n_samples=self.val_n_samples, z_temp=self.val_z_temp, share_znode=self.val_share_znode
            )
            vals.append(pinball_loss_ensemble_joint(Y, samp, self.val_quantiles))
        return float(np.mean(vals)) if len(vals) else float("inf")

    @torch.no_grad()
    def _inv_y(self, ds, y):
        try:
            ...
            if y_np.ndim == 4:
                ...
                return torch.as_tensor(np.stack(outs, axis=0), device=self.device, dtype=torch.float32)
            raise ValueError(f"Unsupported y ndim={y_np.ndim}, shape={y_np.shape}")

    @torch.no_grad()
    def find_best_z_temp(
        self, z_temp_grid, n_samples=50, batch_size=256, quantiles=None, share_znode=None, verbose=True
    ):
        self.G.eval()
        loader = self._make_loader(self.val_set, batch_size=batch_size, shuffle=False)
        quantiles = self.val_quantiles if quantiles is None else list(quantiles)
        share_znode = self.val_share_znode if share_znode is None else bool(share_znode)

        table, best_t, best_pb = [], None, float("inf")
        for t in list(z_temp_grid):
            pbs = []
            for batch in loader:
                X, Y = self._get_batch(batch)
                samp = self.G.sample(X, n_samples=int(n_samples), z_temp=float(t), share_znode=share_znode)
                pb = pinball_loss_ensemble_joint(self._inv_y(self.val_set, Y), self._inv_y(self.val_set, samp), quantiles)
                pbs.append(pb)
            mean_pb = float(np.mean(pbs)) if len(pbs) else float("inf")
            table.append((float(t), mean_pb))
            if verbose:
                print(f"[find_z_temp] z_temp={float(t):.4f} | val pinball(real)={mean_pb:.6f}")
            if mean_pb < best_pb:
                best_pb, best_t = mean_pb, float(t)

        self.best_z_temp = best_t
        self.best_val_pinball_real = best_pb
        self.ztemp_table = table
        if verbose:
            print(f"[find_z_temp] BEST z_temp={best_t} | BEST val pinball(real)={best_pb:.6f}")
        return best_t, best_pb, table

    def fit(self, epochs=1000, batch_size=32, patience=50, best_path=None, verbose=True):
        best_path = best_path or "best_joint_gan_nofm.pt"
        best_val, bad = float("inf"), 0
        best_state_g, best_state_d = None, None
        ...
        for ep in range(1, int(epochs) + 1):
            ...
            for batch in train_loader:
                ...
                for _ in range(self.n_critic):
                    zg, zn = self._sample_latents(B)
                    Y_fake = self.G(X, zg, zn).detach()
                    loss_d = d_hinge_loss(self.D(X, Y), self.D(X, Y_fake))
                ...
                zg, zn = self._sample_latents(B)
                Y_fake = self.G(X, zg, zn)
                loss_adv = g_hinge_loss(self.D(X, Y_fake))

                w_sup = self._sup_weight(ep)
                if w_sup > 0:
                    Ys = []
                    for _ in range(self.sup_samples):
                        zg_s, zn_s = self._sample_latents(B)
                        Ys.append(self.G(X, zg_s, zn_s))
                    Y_mean = torch.stack(Ys, dim=0).mean(dim=0)
                ...
                d_sum += float(loss_d.detach().cpu())
                gadv_sum += float(loss_adv.detach().cpu())
                sup_sum += float(loss_sup.detach().cpu())
                n_steps += 1

            val_pb = self.evaluate_val_pinball(batch_size=max(256, batch_size))
            if verbose and (ep == 1 or ep % 10 == 0):
                denom = max(n_steps, 1)
                print(f"[ep {ep:04d}] train D={d_sum/denom:.6f} G_adv={gadv_sum/denom:.6f} SUP={sup_sum/denom:.6f} | val pinball(norm)={val_pb:.6f}")

            if val_pb < best_val - 1e-8:
                best_val = val_pb
                best_state_g = copy.deepcopy(self.G.state_dict())
                best_state_d = copy.deepcopy(self.D.state_dict())
                bad = 0
                if best_path is not None:
                    torch.save({"G": best_state_g, "D": best_state_d, "best_val_pinball": best_val}, best_path)
            else:
                bad += 1
                if bad >= int(patience):
                    if verbose:
                        print(f"Early stop at epoch {ep}, best_val_pinball={best_val:.6f}")
                    break

        if best_state_g is None and best_path is not None and os.path.exists(best_path):
            ckpt = torch.load(best_path, map_location=self.device)
            best_state_g = ckpt.get("G", ckpt)
            best_state_d = ckpt.get("D", None)

        if best_state_g is not None:
            self.G.load_state_dict(best_state_g)
        if best_state_d is not None:
            self.D.load_state_dict(best_state_d)

        self.G.eval(); self.D.eval()
        return self

def run_gan_joint(
    data_path, node_cols, device=None,
    epochs=1000, batch_size=128, patience=50, seed=42, train_length=8760, val_ratio=0.2, add_lag1=True,
    lr_g=1e-4, lr_d=4e-4, n_critic=3, grad_clip=1.0,
    lambda_adv=2.0, lambda_sup=5.0, sup_samples=8, sup_warmup_epochs=50, sup_loss_type="l1",
    z_global_dim=16, z_node_dim=8, trunk_hidden=256, trunk_blocks=2, head_hidden=256, head_blocks=2, dropout=0.0,
    val_n_samples=50, val_z_temp=1.0, val_share_znode=False, val_quantiles=None,
    find_best_ztemp=True, z_temp_grid=(0.5, 0.8, 1.0, 1.2, 1.5, 2.0, 3.0),
    ztemp_search_n_samples=50, ztemp_search_batch_size=256, ztemp_share_znode=None,
    ckpt_dir="../Model/ckpt_joint_gan", verbose=True,
):
    os.makedirs(ckpt_dir, exist_ok=True)
    device = device or torch.device("cuda" if torch.cuda.is_available() else "cpu")
    set_seed(seed)

    train_set = Dataset_load_multi_node_vae(data_path=data_path, node_cols=node_cols, flag="train", train_length=train_length, val_ratio=val_ratio, seed=seed, add_lag1=add_lag1)
    val_set   = Dataset_load_multi_node_vae(data_path=data_path, node_cols=node_cols, flag="val",   train_length=train_length, val_ratio=val_ratio, seed=seed, add_lag1=add_lag1)
    test_set  = Dataset_load_multi_node_vae(data_path=data_path, node_cols=node_cols, flag="test",  train_length=train_length, val_ratio=val_ratio, seed=seed, add_lag1=add_lag1)

    runner = Runner_gan_joint(
        train_set=train_set, val_set=val_set, test_set=test_set, n_nodes=len(node_cols), device=device,
        z_global_dim=z_global_dim, z_node_dim=z_node_dim, trunk_hidden=trunk_hidden, trunk_blocks=trunk_blocks,
        head_hidden=head_hidden, head_blocks=head_blocks, dropout=dropout,
        lr_g=lr_g, lr_d=lr_d, n_critic=n_critic, grad_clip=grad_clip,
        lambda_adv=lambda_adv, lambda_sup=lambda_sup, sup_samples=sup_samples, sup_warmup_epochs=sup_warmup_epochs,
        sup_loss_type=sup_loss_type, val_n_samples=val_n_samples, val_z_temp=val_z_temp,
        val_share_znode=val_share_znode, val_quantiles=val_quantiles
    )

    best_path = os.path.join(ckpt_dir, f"best_joint_{len(node_cols)}nodes_zg{z_global_dim}_zn{z_node_dim}_JOINT_nofm_mean_{sup_loss_type}.pt")
    runner.fit(epochs=epochs, batch_size=batch_size, patience=patience, best_path=best_path, verbose=verbose)

    best_z_temp, best_val_pinball_real, ztemp_table = float(val_z_temp), None, None
    if bool(find_best_ztemp):
        best_z_temp, best_val_pinball_real, ztemp_table = runner.find_best_z_temp(
            z_temp_grid=z_temp_grid,
            n_samples=int(ztemp_search_n_samples),
            batch_size=int(ztemp_search_batch_size),
            quantiles=(val_quantiles if val_quantiles is not None else runner.val_quantiles),
            share_znode=(ztemp_share_znode if ztemp_share_znode is not None else val_share_znode),
            verbose=verbose,
        )

    joint_G = runner.G.to(device).eval()
    ...
    splits = {"train": {"X": train_set.X, "Y": train_set.y}, "val": {"X": val_set.X, "Y": val_set.y}, "test": {"X": test_set.X, "Y": test_set.y}}
    models_m = {node: joint_G for node in node_cols}; models_m["_JOINT_"] = joint_G
    pack_data_m = {node: {"splits": splits} for node in node_cols}
    pack_data_m["_JOINT_"] = {
        "splits": splits, "node_cols": list(node_cols), "runner": runner, "best_path": best_path,
        "best_z_temp": float(best_z_temp),
        "best_val_pinball_real": (None if best_val_pinball_real is None else float(best_val_pinball_real)),
        "ztemp_table": ztemp_table,
        "val_share_znode": bool(val_share_znode),
        "ztemp_share_znode": (ztemp_share_znode if ztemp_share_znode is not None else val_share_znode),
    }
    return models_m, handlers_m, pack_data_m

@torch.no_grad()
def sample_window_gan_joint(
    models_m, handlers_m, pack_data_m, target_nodes,
    horizon_days=7, start_day=0, n_samples=50, seq_len=24, z_temp=1.0, share_znode=False, split="test"
):
    split = str(split)
    model = models_m["_JOINT_"]
    pack = pack_data_m["_JOINT_"]
    if z_temp is None: z_temp = float(pack.get("best_z_temp", 1.0))
    if share_znode is None: share_znode = bool(pack.get("val_share_znode", False))
    if "splits" not in pack or split not in pack["splits"]:
        raise KeyError(f"pack_data_m['_JOINT_']['splits'] missing split={split}. Available: {list(pack.get('splits', {}).keys())}")

    handler = handlers_m["_JOINT_"][split]
    X_all, Y_all = pack["splits"][split]["X"], pack["splits"][split]["Y"]

    node_names = list(target_nodes); N = len(node_names); D = int(X_all.shape[0])
    ...
    for d in range(start_day, start_day + actual_days):
        y_day_real = handler.inverse_transform_y(Y_all[d])
        truth_concat.append(y_day_real)

        X = X_all[d].unsqueeze(0).to(device).float()
        samp_norm = model.sample(X, n_samples=int(n_samples), z_temp=float(z_temp), share_znode=bool(share_znode)).squeeze(1)
        preds_concat.append(handler.inverse_transform_y(samp_norm))

    y_true = np.concatenate(truth_concat, axis=0)
    y_pred = np.concatenate(preds_concat, axis=1)
    Y_true[:, :L] = y_true[:L].T
    Y_pred[:, :, :L] = np.transpose(y_pred[:, :L, :], (0, 2, 1))

    return {
        "mode": f"single_window_gan_joint_split_{split}",
        "split": split,
        ...
        "z_temp": float(z_temp),
        "share_znode": bool(share_znode),
        "Y_true": Y_true,
        "Y_pred": Y_pred,
    }

class Joint_gan_predictor(nn.Module):
    def __init__(self, joint_model, dataset):
        ...
        if hasattr(dataset, "scaler_y") and hasattr(dataset.scaler_y, "mean_") and hasattr(dataset.scaler_y, "scale_"):
            ...
            mean = np.array([sc.mean_.item() for sc in dataset.scalers_y], dtype=np.float32)
            scale = np.array([sc.scale_.item() for sc in dataset.scalers_y], dtype=np.float32)
        else:
            raise ValueError("Dataset must provide scaler_y (global) or scalers_y (per-node).")

        self.register_buffer("y_mean", torch.from_numpy(mean).view(self.N, 1, 1))
        self.register_buffer("y_scale", torch.from_numpy(scale).view(self.N, 1, 1))
    ...
    def sample(self, X_all, n_samples=50, z_temp=1.0, cpu_offload=False, grad_enabled=None, return_real_scale=True, **kwargs):
        ...
        Y = self.model.sample(X_all, n_samples=int(n_samples), z_temp=float(z_temp), grad_enabled=grad_enabled)  # [S,B,24,N]
        if Y.dim() == 4 and Y.shape[-2] == 24 and Y.shape[-1] == self.N:
            Y = Y.permute(0, 1, 3, 2).contiguous()  # [S,B,N,24]
        else:
            raise ValueError(f"Unexpected GAN sample shape {tuple(Y.shape)}; expected [S,B,24,N].")
...
class DFL_model_gan_Deterministic_SingleNode(nn.Module):
    ...
    def _canon_y_true_11T(self, y_true, T=24):
        if y_true.dim() != 3: raise ValueError(f"y_true must be [B,11,T] or [B,T,11], got {tuple(y_true.shape)}")
    ...
    def _realized_total_cost(self, P_DA_GT, R_up_GT, R_dn_GT, omega_true_T, solver):
        ...
        stage1_res = (R_up_GT * (self.res_up_ratio * bG)[:, None]).sum() + (R_dn_GT * (self.res_dn_ratio * bG)[:, None]).sum()
        sol_rt = self.optnet_RT(R_up_GT.unsqueeze(0), R_dn_GT.unsqueeze(0), omega_true_T.unsqueeze(0), solver=solver)
    ...
    def forward(
        ...
        Y_scen = self.predictor(X_all, n_samples=S, return_real_scale=True, grad_enabled=torch.is_grad_enabled(), **predictor_kwargs).to(device=device, dtype=self.optnet_dtype)
        ...
        B = X_all.shape[0]
        loss_list = []
        aux = {"forecast_sys": [], "omega_true": [], "det_obj": [], "realized_total": [], "rt_obj_true": []} if return_aux else None

        for b in range(B):
            sol_da = self.optnet_DA(forecast_sys[b].unsqueeze(0), solver=solver)
            ...
            if return_aux:
                ...
                aux["det_obj"].append(obj[0].detach() if isinstance(obj, torch.Tensor) else torch.tensor(float("nan"), device=device, dtype=self.optnet_dtype))
        ...
        if return_aux:
            for k in aux: aux[k] = torch.stack(aux[k], dim=0)
...
class DFL_model_gan_Deterministic_MultiNode(nn.Module):
    ...
    def forward(
        ...
        Y_scen = self.predictor(X_all, n_samples=S, return_real_scale=True, grad_enabled=torch.is_grad_enabled(), **predictor_kwargs).to(device=device, dtype=dtype)
        if self.clamp_min is not None:
            Y_scen = torch.clamp(Y_scen, min=float(self.clamp_min))
        ...
        sol_DA = self.optnet_DA(forecast14, solver=solver, return_cost=True)
...
class DFL_model_gan_SingleNode(nn.Module):
    ...
    def forward(self, X_all, y_true, solver=None, return_aux=False, return_filter_aux=False, predictor_n_samples=None, predictor_kwargs=None, filter_kwargs=None):
        solver = solver or self.solver
        device = X_all.device
        predictor_kwargs = dict(predictor_kwargs or {})
        filter_kwargs = dict(filter_kwargs or {})
        ...
        B = X_all.shape[0]
        loss_list = []
        aux = {"forecast_sys": [], "omega_true": [], "saa_obj": [], "realized_total": [], "rt_obj_true": []} if return_aux else None

        for b in range(B):
            sol_da = self.optnet_DA(forecast_sys[b].unsqueeze(0), omega_saa[b].unsqueeze(0), solver=solver, return_rt=False)
            P_DA = sol_da["P_DA"][0].to(dtype=self.optnet_dtype)
            R_up = sol_da["R_up"][0].to(dtype=self.optnet_dtype)
            R_dn = sol_da["R_dn"][0].to(dtype=self.optnet_dtype)
            ...
            if return_aux:
                ...
                aux["omega_true"].append(omega_true[b].detach())
                obj = sol_da.get("obj", None)
                aux["saa_obj"].append(obj[0].detach() if isinstance(obj, torch.Tensor) else torch.tensor(float("nan"), device=device, dtype=self.optnet_dtype))
                aux["realized_total"].append(realized_total.detach())
                aux["rt_obj_true"].append(sol_rt["rt_obj"][0].detach())
...
class DFL_model_gan_MultiNode(nn.Module):
    ...
    def forward(self, X_all, y_true, solver=None, return_aux=False, return_filter_aux=False, predictor_n_samples=None, predictor_kwargs=None, filter_kwargs=None):
        ...
        forecast11 = Y_scen.mean(dim=0)
        scen11 = Y_scen.permute(1, 0, 2, 3).contiguous()
        forecast14 = self._map_11_to_14(forecast11)
...
class DFL_model_gan_DRO_SingleNode(nn.Module):
    ...
    def _to_BT(self, x, B, device, dtype):
        ...
        if x.ndim == 1: x = x.unsqueeze(0).expand(B, -1)
        return x
    ...
    def forward(self, X_all, y_true, hourly_load_min_sys, hourly_load_max_sys, eps, solver=None, return_aux=False, return_filter_aux=False, predictor_n_samples=None, predictor_kwargs=None, filter_kwargs=None):
        solver = solver or self.solver
        device, dtype = X_all.device, self.optnet_dtype
        predictor_kwargs = dict(predictor_kwargs or {})
        filter_kwargs = dict(filter_kwargs or {})
        ...
        om_min_h, om_max_h = Lmin_sys - forecast_sys, Lmax_sys - forecast_sys
        om_min_h, om_max_h = torch.minimum(om_min_h, om_max_h), torch.maximum(om_min_h, om_max_h)
        om_min_s, om_max_s = omega_scen.min(dim=1).values, omega_scen.max(dim=1).values
        om_min, om_max = torch.minimum(om_min_h, om_min_s), torch.maximum(om_max_h, om_max_s)
        ...
        for b in range(B):
            sol_da = self.optnet_DA(
                forecast_sys[b].unsqueeze(0), omega_scen[b].unsqueeze(0),
                om_min[b].unsqueeze(0), om_max[b].unsqueeze(0), eps_t[b].view(1), solver=solver
            )
            P_DA = sol_da["P_DA"][0].to(dtype=dtype)
            R_up = sol_da["R_up"][0].to(dtype=dtype)
            R_dn = sol_da["R_dn"][0].to(dtype=dtype)
            realized_total, _ = self._realized_total_cost(P_DA, R_up, R_dn, omega_true[b], solver=solver)
            loss_list.append(realized_total)
            if return_aux: aux["realized_total"].append(realized_total.detach())
...
class DFL_model_gan_DRO_MultiNode(nn.Module):
    ...
    def forward(self, X_all, y_true, hourly_load_min_sys, hourly_load_max_sys, eps, solver=None, return_aux=False, return_filter_aux=False, predictor_n_samples=None, predictor_kwargs=None, filter_kwargs=None):
        ...
        Y_scen_11 = self.predictor(X_all, n_samples=S, return_real_scale=True, grad_enabled=torch.is_grad_enabled(), **predictor_kwargs).to(device=device, dtype=dtype)
        if self.clamp_min is not None:
        ...
        om_min14, om_max14 = Lmin14 - forecast14, Lmax14 - forecast14
...
def DFL_train(
    ...
    if train_mode == "filter_only" and getattr(dfl, "scenario_filter", None) is not None:
        ...
        if len(optim_params_filter) > 0:
            param_groups.append(
                {
                    "params": optim_params_filter,
                    "lr": float(getattr(args, "filter_lr", 1e-3)),
                    "name": "filter",
                }
            )
            ...
            param_groups.append(
                {
                    ...
                    "lr": float(getattr(args, "dfl_lr", 1e-5)),
                    "name": "predictor",
                }
            )
    ...
    for ep in tqdm(range(epochs), desc=f"Train ({train_mode})", leave=True):
        ...
        epoch_task, epoch_div, ncnt = 0.0, 0.0, 0
        ...
        for X_all, y_true in pbar:
            ...
            kwargs = dict(
                ...
                predictor_kwargs={"z_temp": float(z_temp), "cpu_offload": False},
            ...
            bs = X_all.shape[0]
            epoch_task += float(task_loss_val.detach().cpu()) * bs
            epoch_div += float(div_loss_val.detach().cpu()) * bs
            ncnt += bs
        ...
        train_losses.append(
            {
                "task": epoch_task / max(ncnt, 1),
                "div": epoch_div / max(ncnt, 1),
                "div_type": div_type,
                "lr": [pg["lr"] for pg in optim.param_groups],
...
def DFL_test(
    ...
    for X_all, y_true in pbar:
        ...
        pbar.set_postfix(loss=float(loss_vec.mean().detach().cpu()), BS=test_bs)
...
def run_DFL_gan_joint(
    ...
    if multi:
        ...
        DFL_SO_Class = DFL_model_gan_MultiNode
        DFL_DRO_Class = DFL_model_gan_DRO_MultiNode
        ...
        DFL_SO_Class = DFL_model_gan_SingleNode
        DFL_DRO_Class = DFL_model_gan_DRO_SingleNode
    ...
    # =========================================================
    # Case 1: 不传 stage2_artifact -> 从头跑到 Stage A，再继续 Stage B
    # Case 2: 传入 stage2_artifact -> 直接从 Stage B 开始
    # =========================================================
    if not reused_stage2:
        ...
        def eval_on_splits(model, stage_tag: str):
            ...
            if "test" in eval_splits:
                set_seed(seed)
                out[f"test_losses_{stage_tag}"] = DFL_test(model, test_data, args, z_temp=best_z, problem_mode=problem_mode)
            if "train" in eval_splits:
                set_seed(seed)
                out[f"train_losses_{stage_tag}"] = DFL_test(model, train_data, args, z_temp=best_z, problem_mode=problem_mode)
            return out
        ...
        # ==================== Stage A: random selector train DFL ====================
        ...
        # ---- snapshot: rebuild (NOT deepcopy whole model) ----
        dfl_after_stage2_snapshot = rebuild_model_like(
            src_model=dfl,
            predictor_builder=lambda: build_predictor(models_m, train_data),
    ...
    # ==================== Stage B: optional train filter_only ====================
    if run_stage3:
...
def compare_scenario_filters_with_stage3_learned_gan(
    ...
    """
    Fair comparison under GAN-joint protocol:
    ...
    if verbose:
        print("\n" + "=" * 90)
        print("[Compare Scenario Filters - GAN Joint]")
    ...
    for name in method_names:
        ...
        detail = {
            "method_name": name,
            "scenario_filter_class": (type(scenario_filter_i).__name__ if scenario_filter_i is not None else None),
        }
